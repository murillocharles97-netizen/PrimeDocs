...
function renderDashboard(){
    ...
    const acoes=[...montarAcoesOperacionais({financeiro,pedidos,filamentos,lojasVisitar}),...montarAcoesProducaoAtual()];
    ...
    app.innerHTML=`<div class="operationsDashboard">
        ...
        <section class="operationsActionSection"><div class="operationsSectionTitle"><div><span>PRIORIDADES</span><h2>Ação do dia</h2></div><div class="operationsActionMeta"><small>${acoes.length} ações sugeridas</small>${obterAcoesOperacionaisConcluidas().length?`<button type="button" onclick="resetarAcoesOperacionaisConcluidas()">Resetar concluídas</button>`:""}</div></div>${acoes.length?`<div class="operationsActionList">${acoes.slice(0,10).map(renderAcaoOperacional).join('')}</div>`:`<div class="operationsAllClear"><i data-lucide="circle-check-big"></i><div><strong>Operação em dia</strong><p>Nenhuma pendência crítica encontrada.</p></div></div>`}</section>
...
function montarAcoesOperacionais({financeiro,pedidos,filamentos,lojasVisitar}){const hoje=Utils.hoje(),acoes=[],concluidas=obterAcoesOperacionaisConcluidas();const add=a=>{const id=gerarIdAcaoOperacional({titulo:a.id||a.titulo,descricao:a.descricao,modulo:a.modulo});acoes.push({...a,id});};const produtos=Storage.listarProdutos().filter(p=>p.ativo!==false),clientes=Storage.listarClientes?Storage.listarClientes().filter(c=>c.ativo!==false):[],lojas=Storage.listarLojas().filter(l=>l.ativo!==false),consignados=Storage.listarConsignados(),empresas=Storage.listarEmpresas?Storage.listarEmpresas().filter(e=>e.ativa!==false):[];if(!produtos.length)add({id:"setup-produto",icone:"package-plus",titulo:"Cadastrar primeiro produto",descricao:"Monte seu catálogo para orçamentos, pedidos e consignados.",prioridade:"media",modulo:"produtos",auto:true});if(!clientes.length)add({id:"setup-cliente",icone:"user-plus",titulo:"Cadastrar cliente",descricao:"Crie sua primeira ficha de cliente.",prioridade:"media",modulo:"clientes",auto:true});if(!pedidos.length)add({id:"setup-pedido",icone:"clipboard-plus",titulo:"Criar primeiro pedido",descricao:"Controle produção, pagamento e entrega.",prioridade:"media",modulo:"pedidos",auto:true});if(!lojas.length&&!consignados.length)add({id:"setup-consignado",icone:"store",titulo:"Cadastrar loja parceira",descricao:"Prepare a operação de consignado.",prioridade:"media",modulo:"lojas",auto:true});if(!empresas.some(e=>e.nome&&e.logo))add({id:"setup-empresa",icone:"building-2",titulo:"Configurar empresa e logo",descricao:"Use sua identidade nos PDFs e orçamentos.",prioridade:"media",modulo:"configuracoes",auto:true});financeiro.filter(i=>i.status==='atrasado'||i.vencimento===hoje).forEach(i=>add({id:`financeiro-${i.id||i.origemId||i.clienteNome||i.vencimento}`,icone:'hand-coins',titulo:`Receber ${Utils.moeda(i.valorRestante)}`,descricao:i.clienteNome,prioridade:i.status==='atrasado'?'alta':'media',modulo:'financeiro'}));pedidos.filter(p=>p.statusPedido==='em_producao').forEach(p=>add({id:`produzir-${p.id}`,icone:'printer',titulo:`Produzir pedido de ${p.clienteNome}`,descricao:p.dataEntregaPrevista?`Entrega ${formatarDataBR(p.dataEntregaPrevista)}`:'Sem previsão',prioridade:p.dataEntregaPrevista&&p.dataEntregaPrevista<=hoje?'alta':'media',modulo:'pedidos'}));pedidos.filter(p=>p.statusPedido==='pronto').forEach(p=>add({id:`entregar-${p.id}`,icone:'package-check',titulo:`Entregar pedido de ${p.clienteNome}`,descricao:'Pedido pronto',prioridade:'media',modulo:'pedidos'}));lojasVisitar.forEach(i=>add({id:`conferir-${i.loja.id}`,icone:'store',titulo:`Conferir ${i.loja.nome}`,descricao:`Última visita há ${i.dias} dias`,prioridade:i.dias>=30?'alta':'media',modulo:'conferencia'}));filamentos.filter(f=>Number(f.pesoAtualKg||0)<=Number(f.alertaMinimoKg||0)).forEach(f=>add({id:`filamento-${f.id}`,icone:'shopping-cart',titulo:`Comprar ${f.material} ${f.cor}`,descricao:`Restam ${(Number(f.pesoAtualKg||0)*1000).toFixed(0)} g`,prioridade:Number(f.pesoAtualKg||0)<=0?'alta':'media',modulo:'filamentos'}));return acoes.filter(a=>!concluidas.includes(String(a.id))).sort((a,b)=>(a.prioridade==='alta'?0:1)-(b.prioridade==='alta'?0:1));}
function renderAcaoOperacional(a){return `<article class="operationsAction priority-${a.prioridade}" data-action-id="${escaparHtmlDashboard(a.id)}"><span><i data-lucide="${a.icone}"></i></span><div><strong>${escaparHtmlDashboard(a.titulo)}</strong><small>${escaparHtmlDashboard(a.descricao)}</small></div><b>${a.prioridade==='alta'?'Urgente':a.auto?'Configurar':'Hoje'}</b><section class="operationsActionButtons"><button onclick="navegar('${a.modulo}')">Abrir <i data-lucide="arrow-up-right"></i></button><button class="doneActionButton" onclick="concluirAcaoOperacional('${escaparHtmlDashboard(a.id)}')">Concluído <i data-lucide="check"></i></button></section></article>`;}
