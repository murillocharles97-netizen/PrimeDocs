(function () {
    ...
    function MobileProductCard(p) {
        ...
        return `<div class="mobileProductSwipe" data-product-swipe="${esc(p.id)}"><div class="swipeAction swipeProduce"><i data-lucide="factory"></i><span>Produzir</span></div><div class="swipeAction swipeEdit"><i data-lucide="pencil"></i><span>Editar</span></div><article class="mobileProductCard mobile-product-card ${aberto ? "isExpanded" : ""}" onclick="ProdutosMobile.expandir('${esc(p.id)}')">
            <div class="mobileProductCardMain mobile-product-main">${ProductImage(p)}<div class="mobileProductIdentity"><h3>${esc(p.nome || "Produto sem nome")}</h3><p>${esc(p.codigo || "Sem SKU")} <b>•</b> ${esc(p.colecao?.nome || "Geral")}</p><span class="mobileProductStatus ${classe}" onclick="${p.producao ? `event.stopPropagation();ProdutosMobile.verProducao('${esc(p.id)}')` : ""}"><i data-lucide="${icone}"></i>${esc(status)}</span><div class="mobileProductValues mobile-product-price-row"><strong>${Utils.moeda(p.preco)}</strong><span class="${p.lucro < 0 ? "negative" : ""}">Lucro ${Utils.moeda(p.lucro)}</span></div></div><div class="mobileProductCardActions"><button class="mobileProductMore" onclick="event.stopPropagation();ProdutosMobile.menu('${esc(p.id)}')" aria-label="Mais ações"><i data-lucide="ellipsis-vertical"></i></button><button class="mobileProductProduce mobile-product-produce" onclick="event.stopPropagation();ProdutosMobile.produzir('${esc(p.id)}')"><i data-lucide="circle-play"></i> Produzir</button></div></div>
            <footer class="mobileProductMetrics mobile-product-metrics"><div><i data-lucide="box"></i><span><strong>${p.qtdEstoque} un.</strong><small>Estoque</small></span></div><div><i data-lucide="clock-3"></i><span><strong>${formatarTempo(p.tempoMin)}</strong><small>Impressão</small></span></div><div><i data-lucide="weight"></i><span><strong>${num(p.peso).toFixed(num(p.peso) % 1 ? 1 : 0)}g</strong><small>Filamento</small></span></div><div><i data-lucide="circle-dollar-sign"></i><span><strong>${Utils.moeda(p.custo)}</strong><small>Custo</small></span></div></footer>
