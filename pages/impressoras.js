...
function cardImpressora(item) {
    ...
    return `<article class="erpEntityCard printerCard">
        ...
        <div class="printerCurrent"><span>Operação atual</span><strong>${escaparImpressora(operacao?.nome || "Nenhuma")}</strong>${ordem ? `<small>${escaparImpressora(ordem.produtoNome)} · Pedido #${String(ordem.pedidoId).slice(-5)} · ${escaparImpressora(ordem.clienteNome)}</small>` : ""}${loteAtual ? `<div class="productionProgress"><div><span>${formatarMinutosProducao(Producao.calcularTempoDecorrido(loteAtual))} de ${formatarMinutosProducao(loteAtual.tempoPrevistoMinutos)}</span><strong>${Producao.calcularProgressoLote(loteAtual)}%</strong></div><i><b style="width:${Producao.calcularProgressoLote(loteAtual)}%"></b></i></div>` : ""}</div>
        ...
        <div class="erpCardActions printerActions">${loteAtual ? `<button onclick="navegar('producao')"><i data-lucide="factory"></i> Abrir produção</button>${item.status === "imprimindo" ? `<button onclick="pausarLoteUI('${loteAtual.id}')"><i data-lucide="pause"></i> Pausar</button>` : `<button onclick="retomarLoteUI('${loteAtual.id}')"><i data-lucide="play"></i> Retomar</button>`}<button onclick="abrirModalConcluirLote('${loteAtual.id}')"><i data-lucide="check"></i> Concluir</button><button class="danger" onclick="abrirModalFalhaLote('${loteAtual.id}')"><i data-lucide="triangle-alert"></i> Falha</button>` : `<button onclick="abrirModalImpressora('${item.id}')"><i data-lucide="pencil"></i> Editar</button>${item.status === "manutencao" ? `<button onclick="concluirManutencaoImpressora('${item.id}')"><i data-lucide="check"></i> Concluir manutenção</button>` : `<button onclick="iniciarManutencaoImpressora('${item.id}')"><i data-lucide="wrench"></i> Manutenção</button>`}${["pausada","offline"].includes(item.status) ? `<button onclick="alterarStatusImpressora('${item.id}','livre')"><i data-lucide="play"></i> Liberar</button>` : ""}`}<button onclick="verHistoricoImpressora('${item.id}')"><i data-lucide="history"></i> Histórico</button></div>
    </article>`;
}
