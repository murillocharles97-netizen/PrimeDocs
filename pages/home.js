...
function agruparAcoesOperacionais(acoes){const grupoDe=a=>a.auto?'configuracao':a.prioridade==='alta'?'urgentes':['financeiro','pedidos','conferencia'].includes(a.modulo)&&['Receber','Entregar','Conferir'].includes(a.acao)?'hoje':'semana';const definicoes=[['urgentes','Urgentes'],['hoje','Hoje'],['semana','Esta semana'],['configuracao','Configuração']];return definicoes.map(([id,titulo])=>({id,titulo,acoes:acoes.filter(a=>grupoDe(a)===id)})).filter(g=>g.acoes.length);}
...
function renderAcaoOperacional(a){return `<article class="operationsAction priority-${a.prioridade}" data-action-id="${escaparHtmlDashboard(a.id)}" role="button" tabindex="0" onclick="navegar('${a.modulo}')" onkeydown="if(event.key==='Enter')navegar('${a.modulo}')"><span><i data-lucide="${a.icone}"></i></span><div><strong>${escaparHtmlDashboard(a.titulo)}</strong><small>${escaparHtmlDashboard(a.descricao)}</small></div><b>${a.prioridade==='alta'?'Urgente':a.auto?'Configurar':'Hoje'}</b><section class="operationsActionButtons"><button onclick="event.stopPropagation();navegar('${a.modulo}')">${a.acao||'Abrir'} <i data-lucide="arrow-up-right"></i></button><button class="actionMoreButton" aria-label="Mais ações" onclick="event.stopPropagation();abrirMenuAcaoOperacional('${escaparHtmlDashboard(a.id)}')"><i data-lucide="ellipsis"></i></button></section></article>`;}
function abrirMenuAcaoOperacional(id){const acoes=[...montarAcoesOperacionais({financeiro:Financeiro.sincronizar(),pedidos:Storage.listarPedidos().filter(p=>p.ativo!==false),filamentos:Storage.listarFilamentos().filter(f=>f.ativo!==false),lojasVisitar:calcularLojasParaVisitar()}),...montarAcoesProducaoAtual()];const acao=acoes.find(a=>String(a.id)===String(id));if(!acao)return;Modal.abrir("Ação sugerida",`<div class="compactActionMenu"><button onclick="Modal.fechar();navegar('${acao.modulo}')"><i data-lucide="arrow-up-right"></i><span><strong>${acao.acao||'Abrir'}</strong><small>${escaparHtmlDashboard(acao.titulo)}</small></span></button><button onclick="Modal.fechar();concluirAcaoOperacional('${escaparHtmlDashboard(acao.id)}')"><i data-lucide="check"></i><span><strong>Marcar como concluída</strong><small>Ocultar esta sugestão</small></span></button></div>`);lucide.createIcons();}
