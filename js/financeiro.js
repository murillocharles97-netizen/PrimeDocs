...
function gerarNotificacoesOperacionais() {
    ...
    Storage.listarFilamentos().filter(f=>f.ativo!==false&&Number(f.pesoAtualKg||0)<=Number(f.alertaMinimoKg||0)).forEach(f=>geradas.push(criarNotificacaoOperacional(`filamento-${f.id}`,"Filamento crítico",`${f.material} ${f.cor} · ${(Number(f.pesoAtualKg||0)*1000).toFixed(0)} g`,"filamentos","alta","filamentos",anteriores)));
    calcularLojasParaVisitar().forEach(i=>geradas.push(criarNotificacaoOperacional(`loja-visita-${i.loja.id}`,"Loja para visitar",`${i.loja.nome} · ${i.dias} dias sem conferência`,"consignado","media","conferencia",anteriores)));
