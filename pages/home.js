...
function renderDashboard(){
    ...
    const empresaAtiva=Storage.buscarEmpresaPadrao?.()?.nome||"PrimeLine 3D";
    const usuarioFirebase=window.PrimeFirebase?.auth?.currentUser;
    const nomeUsuario=String(usuarioFirebase?.displayName||usuarioFirebase?.email||"").split(/[ @]/)[0];
    const saudacaoCompleta=`${saudacao}${nomeUsuario?`, ${nomeUsuario}`:""}!`;
    app.innerHTML=`<div class="operationsDashboard">
        <section class="operationsHero"><div><span>${escaparHtmlDashboard(empresaAtiva)} · CENTRAL DE OPERAÇÕES</span><h1>${escaparHtmlDashboard(saudacaoCompleta)}</h1><p>${capitalizarDashboard(agora.toLocaleDateString('pt-BR',{weekday:'long',day:'2-digit',month:'long'}))} · Aqui está o resumo do que acontece na sua empresa hoje.</p></div><button onclick="abrirCentralNotificacoes()" aria-label="Abrir notificações"><i data-lucide="bell-ring"></i><strong>${notificacoes.filter(n=>!n.visualizada).length}</strong><span>notificações</span></button></section>
...
function renderProducaoAtualInicio(){
    const lotes=(Storage.listarLotesExecucao?.()||[]).filter(lote=>["em_execucao","pausada"].includes(lote.status)).slice(0,2);
    if(!lotes.length)return "";
    return `<section class="homeProductionSection"><header><div><span>PRODUÇÃO EM ANDAMENTO</span><h2>Acompanhamento em tempo real</h2></div><button type="button" onclick="navegar('producao')">Ver todas <i data-lucide="arrow-up-right"></i></button></header><div>${lotes.map(lote=>{
        const ordem=Storage.buscarOrdemProducaoPorId?.(lote.ordemProducaoId),impressora=Storage.buscarImpressoraPorId?.(lote.impressoraId);
        const progresso=Producao.calcularProgressoLote?.(lote)||0,decorrido=Producao.calcularTempoDecorrido?.(lote)||0;
        return `<button type="button" class="homeProductionCard" onclick="navegar('producao')"><span class="homeProductionIcon"><i data-lucide="printer"></i></span><div><small>${escaparHtmlDashboard(impressora?.nome||lote.impressoraNome||"Impressora")}</small><strong>${escaparHtmlDashboard(ordem?.produtoNome||"Produção em andamento")}</strong><p>${Number(lote.quantidade||0)} un. · ${formatarMinutosProducao(decorrido)} de ${formatarMinutosProducao(lote.tempoPrevistoMinutos||0)}</p><i><b style="width:${Math.min(100,Math.max(0,progresso))}%"></b></i></div><em>${Math.round(progresso)}%</em></button>`;
    }).join("")}</div></section>`;
}
