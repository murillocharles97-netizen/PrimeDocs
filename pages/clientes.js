...
function confirmarInativarCliente(id){const c=Storage.buscarClientePorId(id);if(!c)return;const vinculos=ERPIntegracao.podeArquivarCliente(id);if(vinculos.pedidos.length){Modal.abrir("Cliente possui pedidos",`<div class="confirmContent"><p><strong>${escaparCliente(c.nome)}</strong> possui ${vinculos.pedidos.length} pedido(s) vinculado(s). Para preservar a integridade, escolha arquivar o cliente com o histórico ou transferir os pedidos.</p><div class="modalActions"><button class="btnSecondary" onclick="Modal.fechar()">Cancelar</button><button class="btnSecondary" onclick="abrirTransferenciaPedidosCliente('${escaparCliente(id)}')">Transferir pedidos</button><button class="btn" onclick="inativarCliente('${escaparCliente(id)}')">Arquivar</button></div></div>`);return;}Modal.abrir("Arquivar cliente",`<div class="confirmContent"><p>Deseja arquivar <strong>${escaparCliente(c.nome)}</strong>? O histórico será preservado.</p><div class="modalActions"><button class="btnSecondary" onclick="Modal.fechar()">Cancelar</button><button class="btn dangerButton" onclick="inativarCliente('${escaparCliente(id)}')">Arquivar</button></div></div>`);}
function abrirTransferenciaPedidosCliente(id){const clientes=Storage.listarClientes().filter(item=>item.ativo!==false&&String(item.id)!==String(id)).sort((a,b)=>String(a.nome).localeCompare(String(b.nome),'pt-BR'));if(!clientes.length)return Toast.show("Cadastre outro cliente antes de transferir os pedidos.");Modal.abrir("Transferir pedidos",`<div class="confirmContent"><p>Selecione quem receberá todos os pedidos e o histórico comercial deste cliente.</p><label class="inputGroup"><span>Cliente de destino</span><select id="clienteDestinoTransferencia"><option value="">Selecione</option>${clientes.map(item=>`<option value="${escaparCliente(item.id)}">${escaparCliente(item.nome)}</option>`).join('')}</select></label><div class="modalActions"><button class="btnSecondary" onclick="Modal.fechar()">Cancelar</button><button class="btn" onclick="confirmarTransferenciaPedidosCliente('${escaparCliente(id)}')">Transferir e arquivar</button></div></div>`);}
function confirmarTransferenciaPedidosCliente(id){const destino=document.getElementById('clienteDestinoTransferencia')?.value;if(!destino)return Toast.show("Selecione o cliente de destino.");try{const total=ERPIntegracao.transferirPedidosCliente(id,destino);Modal.fechar();atualizarListaClientes();Toast.show(`${total} pedido(s) transferido(s) com sucesso.`);}catch(erro){console.error("[Clientes] Falha na transferência",erro);Toast.show(erro.message||"Não foi possível transferir os pedidos.");}}
function inativarCliente(id){try{ERPIntegracao.arquivarCliente(id);Modal.fechar();atualizarListaClientes();Toast.show("Cliente arquivado com histórico preservado.");}catch(erro){console.error("[Clientes] Falha ao arquivar",erro);Toast.show(erro.message||"Não foi possível arquivar o cliente.");}}
